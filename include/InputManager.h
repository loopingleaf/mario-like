...
{
public:
	std::string keymapPath = "data/config/keymap.json";

	InputManager();
	~InputManager();

	/**
	 * \brief Load the json config mapping file in the bindings.
	 */
	void loadConfig();
...
private:
    ...
    /**
     * \brief String name of each input to use in keymap.json
     */
    std::map<std::string, int> nameKey =
    {
        {"A",           sf::Keyboard::A},
        {"B",           sf::Keyboard::B},
        {"C",           sf::Keyboard::C},
        ...
        {"MouseXButton1",    sf::Mouse::XButton1},
        {"MouseXButton2",    sf::Mouse::XButton2}
    };
};
